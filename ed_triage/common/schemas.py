...
class VitalSigns(BaseModel):
    ...
    heart_rate_bpm: Optional[int] = Field(
        None, description="Heart rate in beats per minute")
    respiratory_rate_bpm: Optional[int] = Field(
        None, description="Respiratory rate in breaths per minute")
    blood_pressure_systolic_mmHg: Optional[int] = Field(
        None, description="Systolic blood pressure in mmHg")
    blood_pressure_diastolic_mmHg: Optional[int] = Field(
        None, description="Diastolic blood pressure in mmHg")
    temperature: Optional[str] = Field(
        None, description="Temperature with unit, e.g. '38.2 °C'")
