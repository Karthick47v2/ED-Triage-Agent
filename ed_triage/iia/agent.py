...
def interview_node(state: AgentState):
    ...
    logger.debug(f"Interviewer response: {response.content[:50]}...")
...
def extraction_node(state: AgentState):
    ...
    model = get_llm_iia()
    structured_llm = model.with_structured_output(IntakeSummary)
    ...
    logger.info(f"Extraction complete. Emergency: {intake_data.emergency_detected}")
    if not intake_data.interview_complete:
        intake_data.interview_complete = True
    return {"intake_data": intake_data}


def check_conversation_end(state: AgentState) -> Literal["extractor", "__end__"]:
